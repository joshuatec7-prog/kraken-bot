...
pos, size, entry, trail = {p: None for p in PRODUCTS}, {p: 0 for p in PRODUCTS}, {p: None for p in PRODUCTS}, {p: None for p in PRODUCTS}
...
def log(sym, action, price, qty, reason, after):
    ...
    with LOG_PATH.open("a", newline="") as f: csv.writer(f).writerow(row)
    print("|".join(map(str,row)), flush=True)

def _auth_headers(path, body):
    nonce = str(int(time.time()*1000)); data = body.copy(); data["nonce"] = nonce
    payload = urlencode(data); secret = base64.b64decode(SECRET_B64)
    sig = hmac.new(secret, (nonce+payload).encode(), hashlib.sha256).digest()
    return {"APIKey": KEY, "Authent": base64.b64encode(sig).decode(),
            "Content-Type":"application/x-www-form-urlencoded"}, data

def last_prices():
    r = requests.get(f"{BASE}/tickers", timeout=10); r.raise_for_status()
    mp = {}
    for t in r.json().get("tickers", []):
        sym = str(t.get("symbol","")).upper()
        last = t.get("last") or t.get("markPrice") or t.get("bid") or t.get("ask")
        if sym and last is not None: mp[sym] = float(last)
    out = {}
    for want in PRODUCTS:
        if want in mp: out[want] = mp[want]
        else:
            alt = ("PF_" + want.split("_",1)[-1]) if "_" in want else want.replace("PI_","PF_")
            if alt in mp: out[want] = mp[alt]
    return out

def qty_from_usd(usd): return max(1, round(usd))  # ~1 contract ≈ $1

def place_order(symbol, side, qty, reduce_only=False):
    path = "/sendorder"; body = {"orderType":"mkt","symbol":symbol,"side":side,"size":str(int(qty))}
    ...
    if resp.get("result") != "success": raise RuntimeError(f"ORDER_FAIL {resp}")
    return resp

def open_long(sym, px):
    q = qty_from_usd(usd_per_market); place_order(sym,"buy",q)
...
def open_short(sym, px):
    q = qty_from_usd(usd_per_market); place_order(sym,"sell",q)
...
print(f"START|products={PRODUCTS}|budget={TOTAL_BUDGET_EUR}|SMA={SMA_LEN}|thr=({RISE_THR},{FALL_THR})", flush=True)

while True:
    try:
        pxs = last_prices()
        if DEBUG and pxs:
            any_sym = next(iter(pxs))
            print(f"HB|{any_sym}={pxs[any_sym]:.2f}|pos={pos[any_sym]}", flush=True)

        for sym in PRODUCTS:
            if sym not in pxs: 
                if DEBUG: print(f"MISS|no price for {sym}", flush=True)
                continue
            px = pxs[sym]
            ...
            if TRAIL_PCT>0 and pos[sym]=="long":  trail[sym] = max(trail[sym] or -1e9, px*(1-TRAIL_PCT))
            if TRAIL_PCT>0 and pos[sym]=="short": trail[sym] = min(trail[sym] or  1e9, px*(1+TRAIL_PCT))
            ...
            if pos[sym] is None:
                ...
                close_position(sym, px, "flip_long_to_short"); open_short(sym, px)
            elif pos[sym]=="short" and dev > 0:
                close_position(sym, px, "flip_short_to_long"); open_long(sym, px)
